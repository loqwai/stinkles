...
export const getPrompt = (verbosity: Verbosity = "normal"): string => {
  const basePrompt = `You are a game master for a role playing game. You
  are asked to generate responses from the perspective of a player in the game.
...
export const generateWorld = async ({
  ...
  const initialState: State = {
    ...
    messages: [
      {
        role: "system",
        content: getPrompt("verbose"), // Always generate verbose
      },
    ],
    seed,
    inventory: [],
...
export const interact = async (
  ...
  if (messages.length === 0 || messages[0].role !== "system") {
    messages.unshift({ role: "system", content: getPrompt("verbose") });
  } else {
    // Update existing system message to verbose
    messages[0].content = getPrompt("verbose");
  }
